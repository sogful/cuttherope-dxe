...
namespace CutTheRopeDX.Framework.Media
{
    ...
    internal sealed class SoundMgr : FrameworkTypes
    {
        ...
        public static void PlayMusic(string musicResourceName)
        {
            ...
            // don't restart the same track if it's already playing (e.g. menu -> box selection)
            if (localizedName == _currentMusicName && XnaMediaPlayer.State == XnaMediaState.Playing)
            {
                return;
            }

            StopMusic();
            _currentMusicName = localizedName;
            ...
            {
                // music is non-critical; never let a missing/unsupported track crash the game
            }
        }
        ...
        /// </summary>
        /// <summary>Name of the music track currently playing, used to avoid restarting it.</summary>
