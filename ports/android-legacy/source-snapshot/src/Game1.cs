...
{
    /// <summary>
    /// Main game class that manages the MonoGame lifecycle, input handling, and rendering.
    /// </summary>
...
#if ANDROID
        /// <summary>
        /// Recomputes android sizing when the gl surface changes size. The first surface can arrive
        /// before immersive fullscreen settles (status/nav bars still inset it), so re-running
        /// against the final full-screen surface keeps the letterbox + hud aligned to the real edges.
        /// </summary>
        /// <param name="sender">The window raising the event.</param>
        /// <param name="e">Event arguments for the size change.</param>
        private void AndroidSurfaceChanged(object sender, EventArgs e)
        {
            int w = GraphicsDevice.Viewport.Width;
            int h = GraphicsDevice.Viewport.Height;
            if (w < 90 || h < 90 || (w == _lastSurfaceW && h == _lastSurfaceH))
            {
                return;
            }
            _lastSurfaceW = w;
            _lastSurfaceH = h;
            Global.ScreenSizeManager.InitAndroid(w, h);
            TouchPanel.DisplayWidth = w;
            TouchPanel.DisplayHeight = h;
            Rectangle view = Global.ScreenSizeManager.ScaledViewRect;
            CtrRenderer.OnSurfaceChanged(view.Width, view.Height);
        }

        /// <summary>Last android surface size handled, to ignore duplicate resize events.</summary>
        private int _lastSurfaceW, _lastSurfaceH;
#endif
        /*//////////////////////////// android /////////////////////////////////*/
...
#if ANDROID
            // android is always fullscreen; drive sizing from the actual gl surface
            int surfaceWidth = GraphicsDevice.Viewport.Width;
            int surfaceHeight = GraphicsDevice.Viewport.Height;
            _lastSurfaceW = surfaceWidth;
            _lastSurfaceH = surfaceHeight;
            Global.ScreenSizeManager.InitAndroid(surfaceWidth, surfaceHeight);
            TouchPanel.DisplayWidth = surfaceWidth;
            TouchPanel.DisplayHeight = surfaceHeight;
            CtrRenderer.Java_com_zeptolab_ctr_CtrRenderer_nativeInit(GetSystemLanguage());
            CtrRenderer.OnSurfaceCreated();
            // feed the renderer the LETTERBOXED view size (game aspect), not the raw surface, so the
            // game's own aspect-fit (nativeResize VIEW_OFFSET) doesn't double up with ScreenSizeManager's
            // letterbox and squish the scene / push the hud off-screen on non-16:9 devices.
            Rectangle view = Global.ScreenSizeManager.ScaledViewRect;
            CtrRenderer.OnSurfaceChanged(view.Width, view.Height);
            Window.ClientSizeChanged += AndroidSurfaceChanged;
#else
...
#if ANDROID
            // hardware/gesture back button arrives via the back gamepad button on android
            if (GamePad.GetState(PlayerIndex.One).Buttons.Back == ButtonState.Pressed)
            {
                Application.SharedMovieMgr().Stop();
                _ = CtrRenderer.Java_com_zeptolab_ctr_CtrRenderer_nativeBackPressed();
            }
            CtrRenderer.Java_com_zeptolab_ctr_CtrRenderer_nativeTouchProcess(AndroidTouch.GetTouchLocations());
            if (AndroidTouch.TryGetPrimary(out int touchViewX, out int touchViewY))
            {
                _ = Application.SharedRootController().MouseMoved(CtrRenderer.TransformX(touchViewX), CtrRenderer.TransformY(touchViewY));
            }
#else
