...
namespace CutTheRopeDX.GameMain
{
    ...
    internal sealed class AboutView
    {
        ...
        private static ScrollableContainer BuildAboutContainer(IButtonDelegation buttonDelegate)
        {
            ...
            // Android subproject (this port) section -- draft blurb, move to locales later if desired
            Image subprojectLogo = Image.Image_createWithResID(Resources.Img.SubprojectLogo);
            _ = vBox.AddChild(subprojectLogo);

            Text subprojectInfo = CreateCenteredTextBlock(Application.GetString("ABOUT_SUBPROJECT_INFO"), containerWidth);
            _ = vBox.AddChild(subprojectInfo);
