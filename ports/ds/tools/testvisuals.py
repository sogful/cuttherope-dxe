...
for box in range(17):
    ...
    continuation=Image.new("RGB",(256,192))
    continuation.paste(original,(0,-144))
    continuation.paste(original,(0,48))
    lookup=np.frombuffer(palettedata,dtype=np.uint8,count=32768,offset=(box+3)*(512+32768)+512)
    expected=upperart.indexed(upperart.dim(continuation,17),lookup)
