...
#if ANDROID
            // the hud uses -xOffsetScaled to pull itself in from the letterbox bars, but on android
            // the game already renders into a render target that IS the letterboxed view (the bars are
            // outside it), so that compensation pushes the hud (pause/restart) off the target. desktop
            // never hits this because its window always matches the game aspect (xOffset == 0). zero it.
            xOffsetScaled = 0;
#else
