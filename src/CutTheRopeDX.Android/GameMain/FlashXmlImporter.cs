...
namespace CutTheRopeDX.GameMain
{
    ...
    public static class FlashXmlImporter
    {
...
#if ANDROID
        /// <summary>
        /// Converts an absolute animation xml path into the content-relative path
        /// TitleContainer expects on android (e.g. "content/images/animations/zepto_splash.xml").
        /// </summary>
        private static string ToAndroidContentRelative(string xmlPath)
        {
            string normalized = xmlPath.Replace('\\', '/');
            const string marker = "/images/animations/";
            int idx = normalized.IndexOf(marker, StringComparison.OrdinalIgnoreCase);
            return idx >= 0
                ? Helpers.ContentPaths.RootDirectory + normalized[idx..]
                : normalized;
        }
#endif
        /*//////////////////////////// android /////////////////////////////////*/
        ...
        {
#if ANDROID
            // assets live inside the apk; load via TitleContainer using a content-relative path
            using System.IO.Stream xmlStream =
                Microsoft.Xna.Framework.TitleContainer.OpenStream(ToAndroidContentRelative(xmlPath));
