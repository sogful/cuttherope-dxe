...
namespace CutTheRopeDX.Desktop
{
    ...
    internal sealed class Renderer
    {
        ...
        public static void Init()
        {
            ...
            /*//////////////////////////// android /////////////////////////////////*/
#if ANDROID
            // dimmed repeating tile used to fill the letterbox/pillarbox bars on widescreen devices
            try
            {
                using System.IO.Stream src = Microsoft.Xna.Framework.TitleContainer.OpenStream("content/tile.png");
                using System.IO.MemoryStream ms = new System.IO.MemoryStream();
                src.CopyTo(ms);
                ms.Position = 0;
                s_tileTexture = Texture2D.FromStream(Global.GraphicsDevice, ms);
            }
            catch (Exception)
            {
                s_tileTexture = null;
...
#if ANDROID
        /// <summary>
        /// Fills the whole backbuffer with the dimmed repeating tile so the letterbox/pillarbox bars
        /// around the game view aren't plain black. The game render target is blitted on top afterward.
        /// </summary>
        private static void DrawLetterboxFill()
        {
            if (s_tileTexture == null)
            {
                return;
            }
            Rectangle screen = Global.GraphicsDevice.Viewport.Bounds;
            Global.SpriteBatch.Begin(SpriteSortMode.Deferred, BlendState.Opaque, SamplerState.LinearWrap, null, null);
            // source rect spans the full screen with wrap sampling, so the tile repeats across the bars
            Global.SpriteBatch.Draw(s_tileTexture, screen, screen, s_letterboxTint);
            Global.SpriteBatch.End();
        }
#endif
        /*//////////////////////////// android /////////////////////////////////*/
...
#if ANDROID
        /// <summary>
        /// Dimmed repeating tile drawn behind the game view to fill widescreen letterbox/pillarbox bars.
        /// </summary>
        private static Texture2D s_tileTexture;

        /// <summary>
        /// Tint applied to the letterbox tile (dark gray) so it reads as a subtle border, not full brightness.
        /// </summary>
        private static readonly Color s_letterboxTint = new(70, 70, 70);
#endif
