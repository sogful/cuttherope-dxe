...
namespace CutTheRopeDX.GameMain
{
    ...
    internal sealed class MenuController : ViewController, IButtonDelegation, IMovieMgrDelegate, IScrollableContainerProtocol, ITimelineDelegate
    {
        ...
        public void CreateOptions()
        {
            ...
            // slightly tighter spacing to fit the extra Cheats button
            VBox vBox = new VBox().InitWithOffsetAlignWidth(-22f, 2, SCREEN_WIDTH);
            ...
            _ = vBox.AddChild(c3);
            Button c5 = CreateButtonWithTextIDDelegate(Application.GetString("CHEATS"), MenuButtonId.ShowCheats, this);
            _ = vBox.AddChild(c5);
            _ = baseElement.AddChild(vBox);
        ...
        public BaseElement CreatePackElementforContainer(int n, ScrollableContainer c)
        {
            ...
            else
            {
                if (!isComingSoon)
                {
                    ...
                    image3.anchor = 17;
                    // + one box slot (image.width - 20): the leftmost custom box pushes every pack
                    // one slot to the right, so the om-nom parallax threshold shifts with it.
                    monsterSlot.s = (image.width * (n - 1)) + (-20f * n) + packContainer.x + 50f + (image.width - 20f);
        ...
        /*//////////////////////////// android /////////////////////////////////*/
        /// <summary>
        /// Builds the Custom Box element for the pack carousel: a greyscaled cardboard box that
        /// opens the imported-levels screen. Always available, never the default selection.
        /// </summary>
        /// <returns>The configured custom box touch element.</returns>
        public BaseElement CreateCustomBoxElement(ScrollableContainer c)
        {
            TouchBaseElement touchBaseElement = new()
            {
                delegateValue = this,
                bid = MenuButtonId.CustomBox
            };
            BaseElement baseElement = new();
            baseElement.SetName("boxContainer");
            baseElement.anchor = baseElement.parentAnchor = 12;
            ...
            Image image = Image.Image_createWithResIDQuad(Resources.Img.CustomBoxGray, 0);
            image.DoRestoreCutTransparency();
            image.anchor = image.parentAnchor = 9;

            // real Om Nom + black hole backdrop, revealed by the same scissor parallax as pack boxes.
            // the custom box is carousel position 0 == pack index -1 (one slot left of pack 0).
            MonsterSlot monsterSlot = MonsterSlot.Create(RGBAColor.MakeRGBA(0f, 0f, 0f, 1f));
            monsterSlot.c = c;
            monsterSlot.anchor = 9;
            monsterSlot.parentAnchor = 9;
            monsterSlot.y = image.y;
            _ = baseElement.AddChild(monsterSlot);
            Image monster = Image.Image_createWithResIDQuad(Resources.Img.MenuPackUI, 1);
            monster.DoRestoreCutTransparency();
            monster.anchor = 17;
            monsterSlot.s = (image.width * -2) + 20f + c.x + 50f + (image.width - 20f);
            monsterSlot.e = monsterSlot.s + 1200f;
            monster.x = c.x + monsterSlot.width + -20f - GetPackOffset();
            monster.y = c.y + (SCREEN_HEIGHT / 2f);
            monster.parentAnchor = -1;
            _ = monsterSlot.AddChild(monster);
            ...
            text.scaleX = text.scaleY = 0.75f;
            text.SetAlignment(2);
            text.SetString("?. " + Application.GetString("CUSTOM_BOX_NAME"));
            text.y = 140f;
            _ = image.AddChild(text);

            // focus squish, matching the pack boxes (played when the carousel settles on this box)
            Timeline squish = new Timeline().InitWithMaxKeyFramesOnTrack(4);
            squish.AddKeyFrame(KeyFrame.MakeScale(1, 1, KeyFrame.TransitionType.FRAME_TRANSITION_LINEAR, 0));
            squish.AddKeyFrame(KeyFrame.MakeScale(0.95f, 1.05f, KeyFrame.TransitionType.FRAME_TRANSITION_EASE_OUT, 0.15f));
            squish.AddKeyFrame(KeyFrame.MakeScale(1.05f, 0.95f, KeyFrame.TransitionType.FRAME_TRANSITION_EASE_OUT, 0.2f));
            squish.AddKeyFrame(KeyFrame.MakeScale(1, 1, KeyFrame.TransitionType.FRAME_TRANSITION_EASE_OUT, 0.25f));
            _ = baseElement.AddTimeline(squish);
        ...
        /// <summary>
        /// Builds a wide level-box button (the wide box sprite with the level's filename on it).
        /// </summary>
        /// <param name="label">Display label (filename, already truncated to 10 chars + ellipsis).</param>
        /// <param name="bid">Button id raised on press.</param>
        /// <param name="font">Font used for the label.</param>
        /// <param name="starCount">Number of stars to show (0-3), like a normal level tile.</param>
        /// <returns>The configured wide level button.</returns>
        private Button CreateWideLevelButton(string label, ButtonId bid, FontGeneric font, int starCount)
        {
            Image up = Image.Image_createWithResIDQuad(Resources.Img.MenuLevelSelectionWide, 0);
            Image down = Image.Image_createWithResIDQuad(Resources.Img.MenuLevelSelectionWide, 0);
            foreach (Image img in new[] { up, down })
            {
                Text t = new Text().InitWithFont(font);
                t.SetString(label);
                t.SetAlignment(1);
                t.anchor = t.parentAnchor = 18;
                t.scaleX = t.scaleY = 0.5f;
                t.y = -28f;
                _ = img.AddChild(t);

                // star count along the bottom of the box, like a normal level tile (quad 2 + stars)
                Image stars = Image.Image_createWithResIDQuad(Resources.Img.MenuLevelUi, 2 + starCount);
                stars.anchor = stars.parentAnchor = 34;
                stars.scaleX = stars.scaleY = 0.7f;
                stars.y = -14f;
                _ = img.AddChild(stars);
            }
            down.scaleX = down.scaleY = 0.96f;
            Button button = new Button().InitWithUpElementDownElementandID(up, down, bid);
            button.delegateButtonDelegate = this;
            CTRTexture2D texture = Application.GetTexture(Resources.Img.MenuLevelSelectionWide);
            button.ForceTouchRect(MakeRectangle(texture.quadOffsets[0].X, texture.quadOffsets[0].Y, texture.quadRects[0].w, texture.quadRects[0].h));
            return button;
        }

        /// <summary>
        /// Builds the Custom Box screen: imported levels listed as wide boxes (filename, 10 chars + ellipsis)
        /// with an "Add Level" button, an empty-state message, and a back button.
        /// </summary>
        public void CreateCustomLevelSelect()
        {
            MenuView menuView = new();
            // greyscale menu background (desaturated menu_bgr) to mark the custom box as a distinct, extra area
            BaseElement baseElement = new()
            {
                width = (int)SCREEN_WIDTH,
                height = (int)SCREEN_HEIGHT
            };
            Image grayBg = Image.Image_createWithResIDQuad(Resources.Img.MenuBgrGray, 0);
            grayBg.anchor = grayBg.parentAnchor = 34;
            grayBg.scaleX = grayBg.scaleY = 1.25f;
            grayBg.rotationCenterY = grayBg.height / 2;
            grayBg.passTransformationsToChilds = false;
            _ = baseElement.AddChild(grayBg);

            // spinning square shadow (semi-transparent black) like the other menus; fits the greyscale bg
            Image shadow = Image.Image_createWithResIDQuad(Resources.Img.MenuBgrShadow, 0);
            shadow.anchor = shadow.parentAnchor = 18;
            shadow.scaleX = shadow.scaleY = 2f;
            Timeline shadowSpin = new Timeline().InitWithMaxKeyFramesOnTrack(3);
            shadowSpin.AddKeyFrame(KeyFrame.MakeRotation(45, KeyFrame.TransitionType.FRAME_TRANSITION_LINEAR, 0));
            shadowSpin.AddKeyFrame(KeyFrame.MakeRotation(405, KeyFrame.TransitionType.FRAME_TRANSITION_LINEAR, 75));
            shadowSpin.SetTimelineLoopType(Timeline.LoopType.TIMELINE_REPLAY);
            _ = shadow.AddTimeline(shadowSpin);
            shadow.PlayTimeline(0);
            _ = baseElement.AddChild(shadow);

            FontGeneric font = Application.GetFont(Resources.Fnt.BigFont);
            System.Collections.Generic.List<string> files = CustomLevels.ListLevelFiles();
            if (files.Count == 0)
            {
                // big Om Nom filling the empty space: greeting on open, then looping idle
                try
                {
                    OmNomSkinDefinition skin = OmNomSkinRegistry.GetXmlSkinDefinition(1);
                    FlashXmlTargetAnimationBackend omBackend = new(skin);
                    GameObject omNom = omBackend.TargetObject;
                    omBackend.Initialize(new OmNomGreeter(omBackend));
                    omNom.updateable = true; // ticked by the view's update chain so the animation advances
                    omNom.scaleX = omNom.scaleY = 2.5f;
                    omNom.anchor = omNom.parentAnchor = 18;
                    omNom.y = -80f;
                    omBackend.Play(TargetAnimationState.Greeting);
                    _ = baseElement.AddChild(omNom);
                }
                catch (Exception)
                {
                    // animation is decorative; never let it break the screen
                }

                Text empty = new Text().InitWithFont(font);
                empty.SetAlignment(1);
                empty.SetString(Application.GetString("CUSTOM_NO_LEVELS"));
                empty.anchor = empty.parentAnchor = 18;
                empty.y = 200f;
                _ = baseElement.AddChild(empty);
            }
            else
            {
                const float containerWidth = 1100f;
                const float containerHeight = 980f;
                VBox vBox = new VBox().InitWithOffsetAlignWidth(8f, 2, containerWidth);
                for (int i = 0; i < files.Count; i++)
                {
                    string name = System.IO.Path.GetFileNameWithoutExtension(files[i]);
                    string shortName = name.Length > 10 ? name[..10] + "..." : name;
                    int stars = CustomLevels.CountStars(files[i]);
                    Button b = CreateWideLevelButton(shortName, MenuButtonId.ForCustomLevel(i), font, stars);
                    _ = vBox.AddChild(b);
                }
                ...
                container.shouldBounceHorizontally = false;
                _ = baseElement.AddChild(container);
            }

            // "Add Level" button, bottom-middle (same vertical position as the back button)
            Button addBtn = CreateButtonWithTextIDDelegate(Application.GetString("CUSTOM_ADD_LEVEL"), MenuButtonId.CustomAddLevel, this);
            addBtn.anchor = addBtn.parentAnchor = 34;
            addBtn.y = -40f;
            _ = baseElement.AddChild(addBtn);

            _ = menuView.AddChild(baseElement);
            Button backButton = CreateBackButtonWithDelegateID(this, MenuButtonId.BackFromCustom);
            backButton.SetName("backb");
            backButton.x = Canvas.xOffsetScaled;
            _ = menuView.AddChild(backButton);
            AttachSnowfallOverlay(menuView);
            AddViewwithID(menuView, VIEW_CUSTOM);
        }

        /// <summary>
        /// Rebuilds and shows the Custom Box screen (e.g. after importing files).
        /// </summary>
        public void RefreshCustomView()
        {
            if (GetView(VIEW_CUSTOM) != null)
            {
                DeleteView(VIEW_CUSTOM);
            }
            CreateCustomLevelSelect();
        ...
        public void CreateCheats()
        {
            ...
            _ = menuView.AddChild(baseElement);
            Button backButton = CreateBackButtonWithDelegateID(this, MenuButtonId.BackFromCheats);
            backButton.x = Canvas.xOffsetScaled;
            _ = menuView.AddChild(backButton);
        ...
        /// <summary>
        /// Launches the system file picker to import custom level files.
        /// </summary>
        ...
        /// <summary>
        /// Loads an imported custom level by its index in the custom level list.
        /// </summary>
        /// <param name="index">Index into <see cref="CustomLevels.ListLevelFiles"/>.</param>
        private void LoadCustomLevel(int index)
        {
            System.Collections.Generic.List<string> files = CustomLevels.ListLevelFiles();
            ...
            System.Xml.Linq.XElement map = null;
            try
            {
                map = CustomLevels.LoadLevel(files[index]);
            }
            catch (Exception)
            {
            }
            ...
            ((MapPickerController)GetChild(0)).SetAutoLoadMap("custom");
            ActivateChild(0);
        }
        /*//////////////////////////// android /////////////////////////////////*/
        ...
        public void CreatePackSelect()
        {
            ...
            packContainer.TurnScrollPointsOnWithCapacity(CTRPreferences.GetPacksCount() + 3);
            ...
            customBoxElement.bbc = MakeRectangle(0f, 0f, -20f, 0f);
            scrollPointX += customBoxElement.width + -20f;
            for (int i = 0; i < displayCount; i++)
            {
                TouchBaseElement touchBaseElement = (TouchBaseElement)CreatePackElementforContainer(i, packContainer);
                boxes[i + 1] = touchBaseElement;
            ...
            if (lastPack < 0 || lastPack >= CTRPreferences.GetPacksCount())
            {
                lastPack = 0;
            }
            ((CTRRootController)Application.SharedRootController()).SetBox(CTRPreferences.GetLastGamePack());
            // +1: real pack -> scroll point (custom box occupies scroll point 0)
            packContainer.PlaceToScrollPoint(lastPack + 1);
            ScrollableContainerchangedTargetScrollPoint(packContainer, lastPack + 1);
        }
        ...
        {
            // scroll point 0 is the custom box; real pack p sits at scroll point p+1
            int rp = i - 1;
            currentPack = rp;
            pack = rp;
            scrollPacksLeft = 0;
            ...
            // play the focus squish on the settled box (custom box and coming-soon included)
            boxes[i].GetChildWithName("boxContainer")?.PlayTimeline(0);
            if (rp < 0 || rp >= CTRPreferences.GetPacksCount())
            {
                // custom box (rp < 0) or coming-soon placeholder: no pack data to apply
                return;
            }
            UNLOCKEDSTATE unlockedForPackLevel = CTRPreferences.GetUnlockedForPackLevel(rp, 0);
            ...
            {
                CTRPreferences.SetUnlockedForPackLevel(UNLOCKEDSTATE.UNLOCKED, rp, 0);
                childWithName.PlayTimeline(0);
            }
            CTRRootController cTRRootController = (CTRRootController)Application.SharedRootController();
            if (showNextPackStatus && rp == cTRRootController.GetPack() + 1)
            {
        ...
        public void ScrollableContainerchangedTargetScrollPoint(ScrollableContainer e, int i)
        {
            ...
            if (rp < 0 || rp >= CTRPreferences.GetPacksCount())
            {
                // custom box or coming-soon placeholder: don't persist it as the last pack,
                // so re-entering pack selection never defaults onto a non-pack box.
                return;
            }
            CTRPreferences.SetLastBox(rp);
            CTRPreferences.SetLastGamePack(CTRPreferences.GetBoxForPack(rp));
        }
        ...
        public override void Activate()
        {
            ...
            ShowView(viewToShow);
            // no explicit StopMusic: PlayMusic skips if the same track is already playing
            // (and stops/switches if different), so navigating menu->box won't restart it
        ...
        public void ShowNextPack()
        {
            ...
            {
                packContainer.delegateScrollableContainerProtocol = this;
                // +2: next real pack (currentPackIndex+1) -> its scroll point (+1 for the custom box)
                packContainer.MoveToScrollPointmoveMultiplier(currentPackIndex + 2, 0.8f);
            ...
            {
                packContainer.PlaceToScrollPoint(cTRRootController.GetPack() + 2);
                CTRSoundMgr.StopMusic();
        ...
        public void MoviePlaybackFinished(string url)
        {
            ...
            {
                // +1 throughout: real pack -> scroll point (custom box at scroll point 0).
                // last pack = GetPacksCount()-1 -> scroll GetPacksCount(); first pack 0 -> scroll 1.
                packContainer.PlaceToScrollPoint(CTRPreferences.GetPacksCount());
                packContainer.MoveToScrollPointmoveMultiplier(1, 0.6f);
                CTRPreferences.DisablePlayLevelScroll();
            }
            else
            {
                packContainer.PlaceToScrollPoint(CTRPreferences.GetLastBox() + 1);
            }
            ShowView(5);
            if (isOutro)
            {
        ...
        {
            CTRSoundMgr.StopMusic();
            // a normal pack level is being loaded: ensure no imported custom map leaks into it
            CustomLevels.PendingCustomMap = null;
            CTRRootController ctrrootController = (CTRRootController)Application.SharedRootController();
        ...
        public void OnButtonPressed(MenuButtonId n)
        {
            ...
            if (n.IsCustomLevelButton())
            {
                LoadCustomLevel(n.GetCustomLevelIndex());
                return;
            }
            ...
            switch (n)
            {
                ...
                case var id when id == MenuButtonId.ShowReset:
                    ShowView(4);
                    return;
                case var id when id == MenuButtonId.CustomBox:
                    // like the pack boxes: only enter when centered; otherwise scroll to it first
                    if (currentPack != -1)
                    {
                    ...
                    ShowView(VIEW_CUSTOM);
                    return;
                case var id when id == MenuButtonId.BackFromCustom:
                    ShowView(VIEW_PACK_SELECT);
                    return;
                case var id when id == MenuButtonId.CustomAddLevel:
                    LaunchCustomLevelPicker();
                    return;
                case var id when id == MenuButtonId.ShowCheats:
                    if (GetView(VIEW_CHEATS) != null)
                    {
                        DeleteView(VIEW_CHEATS);
                    }
                    ...
                    ShowView(VIEW_CHEATS);
                    return;
                case var id when id == MenuButtonId.CheatUnlockAll:
                    Cheats.UnlockAllBoxes = !Cheats.UnlockAllBoxes;
                    ApplyCheatChange();
                    RefreshCheatsView();
                    return;
                ...
                case var id when id == MenuButtonId.PackSelect:
                    ...
                    ShowView(5);
                    packContainer.MoveToScrollPointmoveMultiplier(pack + 1, 0.8f);
                    return;
                ...
                case var id when id == MenuButtonId.NextPack:
                    {
                        ...
                        int sp2 = FixScrollPoint(currentPackIndex + 1 + leftScrollCount - scrollPacksRight);
                ...
                case var id when id == MenuButtonId.PreviousPack:
                    {
                        ...
                        int sp3 = FixScrollPoint(currentPackIndex + 1 - rightScrollCount + scrollPacksLeft);
                ...
                default:
                    ...
                    if (n.IsPackButton())
                    {
                        ...
                        if (pack != targetPack)
                        {
                            packContainer.MoveToScrollPointmoveMultiplier(targetPack + 1, 0.8f);
                            return;
                        }
        ...
        {
            base.Update(delta);
            if (CustomLevelImport.NeedsRefresh && activeViewID == VIEW_CUSTOM)
            {
                CustomLevelImport.NeedsRefresh = false;
                RefreshCustomView();
                return;
            }
        ...
        /// <summary>Custom Box (imported levels) view identifier.</summary>
        public const int VIEW_CUSTOM = 12;

        /// <summary>Cheats screen view identifier.</summary>
        public const int VIEW_CHEATS = 13;
        ...
        /// finishes, switches to a looping idle (mirrors Om Nom at the start of a level).
        /// </summary>
        private sealed class OmNomGreeter : ITimelineDelegate
        {
            private readonly ITargetAnimationBackend backend;
            private bool idling;

            public OmNomGreeter(ITargetAnimationBackend b)
            {
                backend = b;
            }
            ...
            public void TimelineFinished(Timeline t)
            {
                ...
                idling = true;
                backend.PlayRandomIdleVariant((min, max) => min);
            }
        }
        /*//////////////////////////// android /////////////////////////////////*/
